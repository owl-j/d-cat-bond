from mysql_database.mysql_manager.helpers.checkers import (
    iterable, iterable_and_not_string)
from mysql_database.mysql_manager.helpers.general import iterable_to_string
...
def insert_query(columns_specified=True, **kwargs):
    ...
    assert iterable_and_not_string(
        kwargs["values"]
    ), "values supposed to be iterable, e.g. [val1, val2, ...]"
...
def multiple_insert_query(columns_specified=True, **kwargs):
    """
